...
package es.allblue.lizardon.smartrotom.entity;

import es.allblue.lizardon.Lizardon;
import es.allblue.lizardon.smartrotom.core.ScreenRights;
import es.allblue.lizardon.smartrotom.utilities.*;
import es.allblue.lizardon.smartrotom.utilities.Rotation;
...
public class TileEntityScreen extends TileEntity {
    ...
    public static class Screen {
        ...
        public void clampResolution() {
            if(resolution.x > Lizardon.INSTANCE.maxResX) {
                float newY = ((float) resolution.y) * ((float) Lizardon.INSTANCE.maxResX) / ((float) resolution.x);
                resolution.x = Lizardon.INSTANCE.maxResX;
                resolution.y = (int) newY;
            }

            if(resolution.y > Lizardon.INSTANCE.maxResY) {
                float newX = ((float) resolution.x) * ((float) Lizardon.INSTANCE.maxResY) / ((float) resolution.y);
                resolution.x = (int) newX;
                resolution.y = Lizardon.INSTANCE.maxResY;
            }
        }
    ...
    public Screen addScreen(BlockSide side, Vector2i size, @Nullable Vector2i resolution, @Nullable EntityPlayer owner, boolean sendUpdate) {
        ...
        ret.side = side;
        ret.size = size;
        ret.url = Lizardon.INSTANCE.homePage;
        ...
        if(!world.isRemote) {
            ...
            if(sendUpdate)
                Lizardon.NET_HANDLER.sendToAllAround(new CMessageAddScreen(this, ret), point());
        }
    ...
    public void requestData(EntityPlayerMP ep) {
        if(!world.isRemote)
            Lizardon.NET_HANDLER.sendTo(new CMessageAddScreen(this), ep);
    }
    ...
    public void setScreenURL(BlockSide side, String url) {
        ...
        url = Lizardon.applyBlacklist(url);
        scr.url = url;
        ...
        if(world.isRemote) {
            ...
            Lizardon.NET_HANDLER.sendToAllAround(CMessageScreenUpdate.setURL(this, side, url), point());
            markDirty();
        }
    }
    ...
    public void removeScreen(BlockSide side) {
        ...
        if(world.isRemote) {
            ...
            Lizardon.NET_HANDLER.sendToAllAround(new CMessageScreenUpdate(this, side), point()); //Delete the screen

        screens.remove(idx);

        if(!world.isRemote) {
            if(screens.isEmpty()) //No more screens: remove tile entity
                world.setBlockState(pos, Lizardon.INSTANCE.blockScreen.getDefaultState().withProperty(BlockScreen.hasTE, false));
            else
                markDirty();
        }
    }
    ...
    public void setResolution(BlockSide side, Vector2i res) {
        ...
        if(world.isRemote) {
            Lizardon.getProxy().screenUpdateResolutionInGui(new Vector3i(pos), side, res);
            ...
            Lizardon.NET_HANDLER.sendToAllAround(CMessageScreenUpdate.setResolution(this, side, res), point());
            markDirty();
        }
    }
    ...
    public void click(BlockSide side, Vector2i vec) {
        ...
        if(world.isRemote)
            ...
            Lizardon.NET_HANDLER.sendToAllAround(CMessageScreenUpdate.click(this, side, CMessageScreenUpdate.MOUSE_CLICK, vec), point());
    }
    ...
    void clickUnsafe(BlockSide side, int action, int x, int y) {
        if(world.isRemote) {
            ...
            Lizardon.NET_HANDLER.sendToAllAround(CMessageScreenUpdate.click(this, side, action, vec), point());
        }
    }
    ...
    public void updateJSRedstone(BlockSide side, Vector2i vec, int redstoneLevel) {
        ...
        if(world.isRemote) {
            ...
            if(sendMsg)
                Lizardon.NET_HANDLER.sendToAllAround(CMessageScreenUpdate.jsRedstone(this, side, vec, redstoneLevel), point());
        }
    }
    ...
    public void handleJSRequest(EntityPlayerMP src, BlockSide side, int reqId, JSServerRequest req, Object[] data) {
        ...
        if(scr == null) {
            ...
            Lizardon.NET_HANDLER.sendTo(new CMessageJSResponse(reqId, req, 403, "Invalid side"), src);
            return;
        }
        ...
        if(!scr.owner.uuid.equals(src.getGameProfile().getId())) {
            ...
            Lizardon.NET_HANDLER.sendTo(new CMessageJSResponse(reqId, req, 403, "Only the owner can do that"), src);
            return;
        }

        if(scr.upgrades.stream().noneMatch(DefaultUpgrade.REDSTONE_OUTPUT::matches)) {
            Lizardon.NET_HANDLER.sendTo(new CMessageJSResponse(reqId, req, 403, "Missing upgrade"), src);
            return;
        }
        ...
        if(req == JSServerRequest.CLEAR_REDSTONE) {
            ...
            Lizardon.NET_HANDLER.sendTo(new CMessageJSResponse(reqId, req, new byte[0]), src);
            ...
            if(x < 0 || x >= scr.size.x || y < 0 || y >= scr.size.y)
                Lizardon.NET_HANDLER.sendTo(new CMessageJSResponse(reqId, req, 403, "Out of range"), src);
            else {
            ...
            Lizardon.NET_HANDLER.sendTo(new CMessageJSResponse(reqId, req, 400, "Invalid request"), src);
    }

    @Override
    public void onLoad() {
        if(world.isRemote) {
            Lizardon.NET_HANDLER.sendToServer(new SMessageRequestTEData(this));
            Lizardon.getProxy().trackScreen(this, true);
        }
    }
    ...
    public void onChunkUnload() {
        if(world.isRemote) {
            Lizardon.getProxy().trackScreen(this, false);
    ...
    public void updateClientSideURL(IBrowser target, String url) {
        for(Screen scr: screens) {
            if(scr.browser == target) {
                boolean blacklisted = Lizardon.isSiteBlacklisted(url);
                scr.url = blacklisted ? Lizardon.BLACKLIST_URL : url; //FIXME: This is an invalid fix for something that CANNOT be fixed
                ...
                if(blacklisted && scr.browser != null)
                    scr.browser.loadURL(Lizardon.BLACKLIST_URL);
