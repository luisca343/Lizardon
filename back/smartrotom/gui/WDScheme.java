...
import es.allblue.lizardon.smartrotom.miniserv.Constants;
import es.allblue.lizardon.smartrotom.miniserv.client.Client;
import es.allblue.lizardon.smartrotom.miniserv.client.ClientTaskGetFile;
...
public class WDScheme implements IScheme {
    ...
    public void getResponseHeaders(ISchemeResponseHeaders resp) {
        ...
        if(status == 0) {
            ...
            if(extPos >= 0) {
                String mime = ((ClientProxy) Lizardon.getProxy()).getMCEF().mimeTypeFromExtension(task.getFileName().substring(extPos + 1));
