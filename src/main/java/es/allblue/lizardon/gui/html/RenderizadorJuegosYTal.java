package es.allblue.lizardon.gui.html;
...
public class RenderizadorJuegosYTal implements IDisplayHandler, IJSQueryHandler {

    public static RenderizadorJuegosYTal INSTANCE;
    ...
    private PantallaJuegosYTal backup = null;
    private API api;
    ...
    public void setBackup(PantallaJuegosYTal bu) {
        backup = bu;
    }
    ...
    public void showScreen(String url) {
        if(mc.currentScreen instanceof PantallaJuegosYTal)
            ((PantallaJuegosYTal) mc.currentScreen).loadURL(url);
        else if(hasBackup()) {
            mc.displayGuiScreen(backup);
            backup.loadURL(url);
            backup = null;
        } else
            mc.displayGuiScreen(new PantallaJuegosYTal(url));
    }

    public IBrowser getBrowser() {
        if(mc.currentScreen instanceof PantallaJuegosYTal)
            return ((PantallaJuegosYTal) mc.currentScreen).browser;
        else if(backup != null)
    ...
    public void onTick(TickEvent ev) {
        if(ev.phase == TickEvent.Phase.START && ev.side == Side.CLIENT && ev.type == TickEvent.Type.CLIENT) {
            ...
            /*if(key.isPressed() && !(mc.currentScreen instanceof PantallaJuegosYTal)) {
                //Display the web browser UI.
                mc.displayGuiScreen(hasBackup() ? backup : new PantallaJuegosYTal());
                backup = null;
            }*/
        }
    }
    ...
    public void onAddressChange(IBrowser browser, String url) {
        ...
        if(mc.currentScreen instanceof PantallaJuegosYTal)
            ((PantallaJuegosYTal) mc.currentScreen).onUrlChanged(browser, url);
        else if(hasBackup())
