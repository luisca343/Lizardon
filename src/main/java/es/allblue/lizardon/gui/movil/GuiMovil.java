...
public class GuiMovil extends WDScreen {
    ...
    public GuiMovil(ClientProxy.PadData pad) {
        this.pad = pad;
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float ptt) {
        ...
        glColor4f(0.925f, 0.423f, 0.309f, 1.0f); //glColor4f(0.73f, 0.73f, 0.73f, 1.0f);
    ...
    public void handleInput() {
        while(Keyboard.next()) {
            ...
            if(Keyboard.getEventKey() == Keyboard.KEY_ESCAPE) {
                ...
                /*
                if(!pad.view.getURL().contains("lizardon"))
                pad.view.goBack();*/
            }else if(Keyboard.getEventKey() == Keyboard.KEY_RIGHT){
                /*
                if(!pad.view.getURL().contains("lizardon"))
                pad.view.goForward();*/
            }else if(Keyboard.getEventKey() == Keyboard.KEY_HOME){
                pad.view.loadURL(Lizardon.getProxy().pasarParametros(Lizardon.URL_BASE));
            }else if(Keyboard.getEventKey() == Keyboard.KEY_F5){
                pad.view.loadURL(Lizardon.getProxy().pasarParametros(pad.view.getURL()));
            }
        ...
        while(Mouse.next()) {
            ...
            if(pad.view != null && sx >= vx && sx <= vx + vw && sy >= vy && sy <= vy + vh) {
                ...
                if(pressed){
                    GestionarBotones.comprobarBotón(sx, sy, pad.view.getURL());
