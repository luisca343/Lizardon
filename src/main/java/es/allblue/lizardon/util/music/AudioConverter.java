...
public class AudioConverter {
    ...
    public static short[] convertMp3(Path file) throws IOException, UnsupportedAudioFileException {
        Mp3Decoder mp3Decoder = LizardonVoicechatPlugin.SERVER_API.createMp3Decoder(Files.newInputStream(file));
        if (mp3Decoder == null) {
            throw new IOException("Error creating mp3 decoder");
        }
        byte[] data = LizardonVoicechatPlugin.SERVER_API.getAudioConverter().shortsToBytes(mp3Decoder.decode());
